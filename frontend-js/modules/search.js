...
export default class Search {
    ...
    keyPressHandler() {
        ...
        if(searchValue !== "" && searchValue !== this.previousValue) {
            clearTimeout(this.typingWaitTimer)
            this.showLoaderIcon()
            this.hideResultsArea()
    ...
    renderResultsHTML(posts) {
        if (posts.length) {
            ...
            this.resultsArea.innerHTML = `<p class="alert alert-danger text-center shadow-sm">Sorry, we could not find any matching post for that search!</p>`
        }
        this.hideLoaderIcon()
